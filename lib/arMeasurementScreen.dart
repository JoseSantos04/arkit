...
class _ArMeasurementScreenState extends State<ArMeasurementScreen>{
  ...
  onARViewCreated(ARKitController arKitController){
    ...
    this.arKitController.onARTap = (List<ARKitTestResult> ar) {
      final planeTap = ar.firstWhere(
        ...
        orElse: () => null);
      if (planeTap != null){
      onPlaneTapHanlder(planeTap.worldTransform);}
    };
  }
  ...
  void addPlane(ARKitController controller, ARKitPlaneAnchor anchor){
    ...
    plane = ARKitPlane(
      ...
      materials: [
        ARKitMaterial(
          transparency: 0.5,
          diffuse: ARKitMaterialColor(Colors.white)
        )
      ]
    );
  ...
  void onPlaneTapHanlder(Matrix4 transform){
    ...
    final material = ARKitMaterial(
      ...
      diffuse: ARKitMaterialColor(Color.fromRGBO(255, 153, 83, 1))
    );
  ...
  void drawText(String textDistance, vector.Vector3 point){
    final textGeometry = ARKitText(
      ...
      materials: [
        ARKitMaterial(
          diffuse: ARKitMaterialColor(Colors.red)
        )
      ]
    );
